...
import styles from './verifyVTIman.less';
...
const VerifyVTIman = prop => {
  ...
  const getBaseUrlByMode = (mode: string) => {
    if (mode === undefined || mode === null || mode === "prod" || mode === "production") {
      // production
      return process.env.API_VERIFY_VTIMAN_PROD;
    } else if (mode === "alpha") {
      // alpha
      return process.env.API_VERIFY_VTIMAN_ALPHA;
    } else if (mode === "stag" || mode === "staging") {
      // staging
      return process.env.API_VERIFY_VTIMAN_STAGING;
    } else if (mode === "dev" || mode === "develop") {
      // develop
      return process.env.API_VERIFY_VTIMAN_DEV;
    } else {
      // default
      return process.env.API_VERIFY_VTIMAN_PROD;
    }
