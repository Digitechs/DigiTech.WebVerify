...
import styles from './verifyDigiGroup.less';
...
const VerifyDigiGroup = prop => {
  ...
  const fetchCheckTokenVeify = async () => {
    if (isProdMode()) {
      const res = await axios.get(`${process.env.API_VERIFY_DIGI_GROUP_PROD}/auth/active-user?token=${token}&email=${email}`)
      setSuccess(res.data.success);
    } else {
      const res = await axios.get(`${process.env.API_VERIFY_DIGI_GROUP_DEV}/auth/active-user?token=${token}&email=${email}`)
      setSuccess(res.data.success);
    }
  }
