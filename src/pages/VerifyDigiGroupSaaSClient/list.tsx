...
const VerifyDigiGroupSaaSClient = prop => {
  ...
  const fetchCheckTokenVeify = async () => {
    ...
    if (isProdMode()) {
      const res = await axios.get(`${process.env.API_VERIFY_DIGI_GROUP_SAAS_PROD}/client/verify?token=${token}`, { headers: header })
      setSuccess(res.data.success);
    } else {
      const res = await axios.get(`${process.env.API_VERIFY_DIGI_GROUP_SAAS_DEV}/client/verify?token=${token}`, { headers: header })
      setSuccess(res.data.success);
    }
  }
...
export default VerifyDigiGroupSaaSClient
