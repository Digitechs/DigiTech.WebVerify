...
import styles from './verifyBKGolf.less';

const VerifyBKGolf = prop => {
  ...
  const fetchCheckTokenVeify = async () => {
    if (isProdMode()) {
      const res = await axios.get(`${process.env.API_VERIFY_BKGOLF_PROD}/auth/active-user?token=${token}&email=${email}`)
      setSuccess(res.data.success);
    } else {
      const res = await axios.get(`${process.env.API_VERIFY_BKGOLF_DEV}/auth/active-user?token=${token}&email=${email}`)
      setSuccess(res.data.success);
    }
  }
  ...
  return (
    <>
      <Helmet>
        <title>{isProdMode() ? "BK_GOLF" : "Digitech Solutions" }</title>
        ...
        <>
          <div className={styles.wrapper}>
            <div className={styles.inner}>
              <div><img  className={isProdMode() ? styles.imageLogo : ""} src={isProdMode() ? "/logo_bk2.png" : "/logo_digi.png"} alt="Lien he" /></div>
